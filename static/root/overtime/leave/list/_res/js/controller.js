var app = angular.module('leaveList', ['ng-pagination','toastr']);
app.controller('leaveListCtrl',function($scope,leaveSer,toastr,$stateParams,$state,$location){
...
/*    //搜索字段
    ...
    function activatePage(page) {
        ...
        leaveSer.leaveList(listData).then(function(response){
...
//分页
    ...
    leaveSer.Countleave().then(function(response){
        if(response.data.code==0){
    ...
    $scope.cancel = function(){//取消删除
        $scope.delShow = false;
        $state.go('root.overtime.leave.list[12]',{id:null,name:null});
    };
    ...
    $scope.delFn = function(){//确认删除
        ...
        leaveSer.leaveDelete(data).then(function(response){
            if(response.data.code==0){
                count++;
                ...
                if(($scope.custom.itemsCount-count)%10){
                    ...
                    $state.go('root.overtime.leave.list[12]',{id:null,name:null,page:$location.search().page-1});
                }
            }else{
                toastr.error( response.data.msg, '温馨提示');
            }
