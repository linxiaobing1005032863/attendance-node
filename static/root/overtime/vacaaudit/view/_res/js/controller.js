var app = angular.module('auditView', ['toastr']);
app.controller('auditViewCtrl', function($scope,auditSer,$stateParams,toastr,$http,$state){
    var infoEdit ={id: $stateParams.id}
    //获取ID
    auditSer.findtesetId(infoEdit).then(function(response){
        if(response.data.code==0){
            $scope.edit = response.data.data;
            var pathData  = {id:$scope.edit.uuid};
            //获取ID
            auditSer.auditTeset(pathData).then(function(response){
                if(response.data.code== 0){
    ...
    $scope.view  =function(){
        ...
        if($scope.encloSigning){
            ...
            if($stateParams.view == '1'){
                $state.go('root.overtime.vacaaudit.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.overtime.vacaaudit.view[12]',{view:'1'});
            }
        }
    };
    ...
    $scope.fn = function(){//下载
        for(var i=0,len=$scope.encloSigning.length;i<len;i++){
            if($scope.encloSigning[i].checked && !$scope.encloSigning[i].delel){
                ...
                iframe.src=`/vacate/download${encode(obj,true)}`;
