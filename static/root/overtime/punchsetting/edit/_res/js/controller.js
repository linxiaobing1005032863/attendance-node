var app = angular.module('punchsettingEdit', ['toastr','angularjs-dropdown-multiselect']);
app.controller('punchsettingEditCtrl', function($scope, punchsettingSer,$state,toastr,$stateParams,$http){
    var infoEdit ={id: $stateParams.id}
    //根据id
    punchsettingSer.findpunchsetId(infoEdit).then(function(response){
...
//获取全部部门
    punchsettingSer.Alldeparts().then(function(response){
        if(response.data.code == 0){
    ...
    //获取汇总对象
    punchsettingSer.Alluser().then(function(response){
        if(response.data.code == 0){
...
//添加
    $scope.extraAddFun = function(){
        ...
        punchsettingSer.Editpunchset(data).then(function(response){
            if(response.data.code == 0){
