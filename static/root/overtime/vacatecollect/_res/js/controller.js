var app = angular.module('collectModule', [{
    files:[
        "root/overtime/vacatecollect/_res/js/service.js",
    ]
}]);
app.controller('colModuleCtrl',function ($scope,$state) {
    if ($state.current.url == '/vacatecollect') {//默认加载列表
        $state.go('root.overtime.vacatecollect.collect[12]')
    }
    $scope.$emit('isVi',true);//判断是否出现搜索按钮
}).controller('colMenuCtrl',function($scope,$state,$rootScope,$location,collectSer){
