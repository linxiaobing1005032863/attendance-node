var app = angular.module('financesetEdit', ['toastr','angularjs-dropdown-multiselect']);
app.controller('financesetEditCtrl', function($scope, financesetSer,$state,toastr,$stateParams,$http){
    var infoEdit ={id: $stateParams.id}
    //根据id
    financesetSer.findfinancesetId(infoEdit).then(function(response){
...
//添加
    $scope.extraAddFun = function(){
        ...
        financesetSer.Editfinanceset(data).then(function(response){
            if(response.data.code == 0){
