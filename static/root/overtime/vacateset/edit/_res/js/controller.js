var app = angular.module('tesetEdit', ['toastr','angularjs-dropdown-multiselect']);
app.controller('tesetEditCtrl', function($scope, tesetSer,$state,toastr,$stateParams,$http){
    ...
    $scope.extraAddFun = function(){
        ...
        tesetSer.Editteset(data).then(function(response){
            if(response.data.code == 0){
                $state.go('root.overtime.vacateset.list[12]');
                toastr.success("已成功补录", '温馨提示');
            }else{
                toastr.error( response.data.msg, '温馨提示');
            }
        });
