var app = angular.module('tesetView', ['toastr']);
app.controller('tesetViewCtrl', function($scope,tesetSer,$stateParams,toastr,$http,$state){

  /*  ;*/
    var infoEdit ={id: $stateParams.id}
    //获取ID
    tesetSer.findtesetId(infoEdit).then(function(response){
        if(response.data.code==0){
            $scope.edit = response.data.data;
            var pathData  = {id:$scope.edit.uuid};
            //获取ID
            tesetSer.viewTeset(pathData).then(function(response){
                if(response.data.code== 0){
    ...
    $scope.view  =function(){
        ...
        angular.element('.checked-none').attr('checked',false);//清空全选框
            ...
            if($stateParams.view == '1'){
                $state.go('root.overtime.vacateset.view[12]',{view:'2'});
            }else if($stateParams.view == '2'){
                $state.go('root.overtime.vacateset.view[12]',{view:'1'});
            }
        }
    };
